...
const ViewBill = () => {
    ...
    const fetchPayments = async () => {
        ...
        try {
            const response = await fetch(API_URL + '/get_payments.php?invoice_id=' + invoiceId, {
                method: 'GET',
                credentials: 'include',
            })
            const result = await response.json();
                    ...
                    <div className="container-fluid">
                        <div className="row mb-2">
                            <div className="col-sm-6">
                                <h1 className="m-0">Bill Details</h1>
                            </div>
                            {/* /.col */}
                        </div>
                        {/* /.row */}
                                ...
                                <table className="w-100">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Total</th>
                                            <td>{<Numeral value={bill.total.toString()} format={'0,0.0'} />}</td>

                                            <th>Total Paid</th>
                                            <td>{<Numeral value={bill.totalPaid.toString()} format={'0,0.0'} />}</td>

                                            <th>Due</th>
                                            <td>{<Numeral value={bill.due.toString()} format={'0,0.0'} />}</td>
                                        </tr>
                                    </thead>
                                </table>
                            </div>
